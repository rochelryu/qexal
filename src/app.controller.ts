...
export class AppController {
  ...
  async chargerProfil(
    ...
    const user = await this.usersService.verifyUserApi(
      parseInt(id, 10),
      recovery,
    );
    if (user.etat) {
      res.json({
        etat: user.etat,
        result: { name: user.result.name, email: user.result.email },
      });
    } else {
  ...
  async detailsCountriesCallingCode(
    @Request() req,
    @Res() res: Response,
    @Param() params,
  ) {
  ...
  async viewGeneratedPassword(@Request() req, @Res() res: Response) {
    if (req.session.qexal) {
      const { result: user } = await this.usersService.getUserByItem({
  ...
  @Post('/login')
  async login(
    @Body() user: { number: string; password: string },
    @Request() req,
    @Res() res: Response,
  ) {
    if (req.session.qexal) {
      ...
      const { etat, result, error } =
        await this.usersService.verifyUserForConnect(
          user.number.trim(),
          user.password.trim(),
        );
      if (etat) {
  ...
  async signupPostApi(
    ...
    if (user.password.length <= 4) {
      ...
      if (etat) {
        ...
        res.json({
          etat: true,
          result: { name, currencies, recovery, id, language, alpha2code },
        });
      } else {
  ...
  async addSugestion(
    @Body()
    user: { note: string; message: string; id: string; recovery: string },
    @Request() req,
    @Res() res: Response,
  ) {
    const { etat, result, error } = await this.usersService.verifyUserApi(
      parseInt(user.id, 10),
      user.recovery,
    );
    if (etat) {
      await this.usersService.setNotification(
        result.id,
        parseFloat(user.note),
        user.message,
      );
  ...
  async changePasswordApi(
    ...
    const client = await this.usersService.verifyUserApi(
      parseInt(user.id, 10),
      user.recovery,
    );
    const { etat, result, error } =
      await this.usersService.getUserForChangePass(
        client.result.id,
        user.newPassord,
        user.oldPassword,
      );
    if (etat) {
      ...
      await this.usersService.updateUser(client.result.id, {
