...
import {
  ...
  messageOfDelevryCash,
  nextStepForPackaheUpgrade,
  sendMail,
  messageOfFraudeForqexal,
  messageOfDelevryCashForHopInvest,
  payementVerifyByPromo,
  messageOfDelevryCashForBillionaryInvest,
  sendMailBillionary,
  sendMailStartUp,
  getValueOfEthInUsd,
  formatInitDemande,
  fakeDataCountry,
  messageSuccessInvestment,
} from 'src/common/functions/helper';
import { formatDateAndTime, verifyWeekend } from 'src/common/functions/date';
...
import {
  ...
  EMAIL_START_UP,
  NUMBER_OF_USE_NEXT_WEEK,
  PayementByqexalExchange,
  POURCENTAGE_ADMIN,
  POURCENTAGE_BUSINESS,
  POURCENTAGE_PARRAIN,
  POURCENTAGE_PAYNET,
  POURCENTAGE_PAY_BY_WEEK,
  RATE_CRYPTO_INVERSION,
  Reabonnement,
} from 'src/common/constant/constant';
import { hash } from 'bcrypt';
import { differenceInSeconds, subDays, addDays, startOfWeek, format } from 'date-fns';
...
export class UsersController {
  ...
  async dashboardSecond(@Request() req, @Res() res: Response) {
    ...
    lastDemande = await this.service.getLastDemandeForSuivieByItem({
      userid: user.result.id,
      etatid: 2,
    });
    ...
    for (const movie of allMovies.result) {
      const allMovie = await this.service.verifyIfUserAlreadySeeMovie(
        movie.id,
        user.result.id,
      );
    ...
    const numberDemande = await this.service.getCountDemandeByUserInRange(
      new Date(user.result.endContratTime),
      user.result.id,
    );
    toDay.setDate(toDay.getDate() - 1);
    const allDemandeYesterday =
      await this.service.getAllDemandesYesterday(toDay);
    const percentage =
      user.result.accord === 0
        ? (numberDemande.result * 100) / 2
        : (numberDemande.result * 100) /
          (user.result.isEligibleForNextWeek === 1 ? 6 : 5);
    ...
    for (const value of fakeData) {
      ...
      const indexOfCountry = dataOfDemographie.findIndex(
        (country) => country[0] === rowData[0],
      );
      if (indexOfCountry !== -1) {
        dataOfDemographie[indexOfCountry][1] += rowData[1];
      } else {
        dataOfDemographie.push(rowData);
      }
    }
    ...
    if (req.session.qexal) {
      ...
      if (user.etat) {
        if (demande.txHash) {
          ...
          const txHashAlreadyExist = await this.service.getControlCodeItem({
            txHash: demande.txHash.trim(),
          });
          if (txHashAlreadyExist.etat) {
            ...
            const verifyTxHashInfo = await this.service.verifyTxHash(
              user.result.id,
              demande.txHash,
              amountAtPay,
            );
            if (
              verifyTxHashInfo.etat &&
              forfait.result.min - 1 <=
                verifyTxHashInfo.result.montant_net_send + user.result.soldeGain
            ) {
              //Create Demande of Client
              const initDemande = formatInitDemande(
                forfait.result.id,
                forfait.result.numberDayTotalVersement,
                user.result.id,
                verifyTxHashInfo.result.montant_net_send +
                  user.result.soldeGain,
                forfait.result.commissionTotal,
                demande.txHash.trim(),
              );
              const { etat, error } =
                await this.service.setDemande(initDemande);
              await this.service.updateUser(user.result.id, {
                soldeGain: 0,
                soldeInvestissement: demande.amount,
              });
              if (etat) {
                await this.service.setControlCode(
                  user.result.id,
                  ControleCode.success,
                  demande.txHash.trim(),
                  verifyTxHashInfo.result.montant_net_send,
                );
              ...
              return {
                etat: false,
                error:
                  'Votre transaction ne rentre pas dans les règles de cette action donc contacté le support pour remboursement.',
              };
            }
          }
        } else {
          if (
            user.result.soldeGain >= demande.amount &&
            forfait.result.min <= demande.amount
          ) {
            if (user.result.soldeInvestissement <= demande.amount) {
              ...
              const { etat, error } =
                await this.service.setDemande(initDemande);
              await this.service.updateUser(user.result.id, {
                soldeGain: user.result.soldeGain - demande.amount,
                soldeInvestissement: demande.amount,
              });
              if (etat) {
                //Send Mail at client for signal valid transaction
              ...
              return {
                etat: false,
                error: `Votre action minimum d'investissement doit être $${user.result.soldeInvestissement}`,
              };
            }
          } else {
            return {
              etat: false,
              error: 'Client not authorised to this action...',
            };
          }
        }
      } else {
  ...
  async setRecompenseMovie(@Body() body: { playerId: string }, @Request() req) {
    if (req.session.qexal) {
      ...
      const movie = await this.service.getMovieByItem({
      ...
      if (user.etat && movie.etat && user.result.inscription <= 50) {
        ...
        if (
          !userAlreadyViewMovie.etat &&
          userAlreadyViewMovie.error.message === 'Aucune vidéo trouvé'
        ) {
          await this.service.setMovieVisualisation(
            movie.result.id,
            user.result.id,
          );
          const allMovies = await this.service.getAllMoviesByItem({});
          const bonus = parseFloat((1 / allMovies.result.length).toFixed(2));
          await this.service.updateUserForPayeSoldeGainIncrement(
            user.result.id,
            bonus,
          );
          await this.service.updateUser(user.result.id, {
            inscription: user.result.inscription + 1,
          });
          return {
            etat: true,
            result: { bonus, soldeGain: user.result.soldeGain + bonus },
          };
        } else {
          ...
          return {
            etat: false,
            error:
              "Un problème est survenu, veuillez reprendre s'il vous plait.",
          };
        }
      } else {
        return {
  ...
  async payeDemandeAuto(demandeId: number) {
    try {
      ...
      const user = await this.service.getUserByItem({
        id: demande.result.userid,
      });
      //Get diffDay to Moment at create_at demande
      const diffInSecond = differenceInSeconds(
        new Date(),
        new Date(demande.result.last_date_payement),
      );
      const differenceDay = parseInt((diffInSecond / 86400).toString(), 10);
      if (differenceDay > 0) {
        const cumulPercentage =
          demande.result.cumulPercentage +
            differenceDay * demande.result.commissionDay <=
          demande.result.percentageTotal
            ? demande.result.cumulPercentage +
              differenceDay * demande.result.commissionDay
            : demande.result.percentageTotal;
        ...
        const soldeGain =
          ...
          demande.result.percentageTotal
            ? user.result.soldeGain +
              differenceDay *
                demande.result.commissionDay *
                demande.result.amount
            : user.result.soldeGain +
              (demande.result.percentageTotal -
                demande.result.cumulPercentage) *
                demande.result.amount;
        await this.service.updateDemande(demandeId, {
          cumulPercentage,
          etatid: cumulPercentage === demande.result.percentageTotal ? 3 : 2,
          last_date_payement: addDays(
            new Date(demande.result.last_date_payement),
            differenceDay,
          ),
        });
  ...
  async retrait(@Request() req, @Res() res: Response) {
    if (req.session.qexal && req.session.qexal.roleid === 3) {
      ...
      res
        ...
        .render('retrait2', {
          user: user.result,
          paycheckDemande: inWaitPayement.result,
          countNotif,
          notifications,
  ...
  async addForfait(
    ...
    if (req.session.qexal && req.session.qexal.roleid === 3) {
      const newForfait = await this.service.setForfait(
        parseFloat(forfait.montant),
        parseFloat(forfait.inscription),
        forfait.name,
        forfait.color,
      );
  ...
  async emailrec(@Body() demande: RecoveryDto) {
    ...
    if (user.etat) {
      ...
      const updateUser = await this.service.updateUser(user.result.id, {
  ...
  async recovery(@Body() demande: { recovery: string }) {
    const user = await this.service.getUserByItem({
      recovery: demande.recovery.trim(),
    });
    if (user.etat) {
      const newRecovery = await generateRecovery(user.result.recovery);
      const newRecoveryAgain = await generateRecovery(newRecovery);
      const password = await hash(
        newRecoveryAgain.trim() + newRecovery.trim(),
      ...
      return {
        etat: false,
        error: 'Incorrect code, Please verified your mail',
      };
    }
  }
      ...
      if (user.result.isActive) {
        if (user.etat && user.result.soldeGain > 0) {
          ...
          return {
            etat: true,
            result:
              'Your Withdrawal Request is registered. Admins will contact you...',
          };
        } else {
          return {
            etat: false,
            error:
              'You cannot request a withdrawal because the balance is insufficient.',
          };
        }
      } else {
  ...
  async settings(@Request() req, @Res() res: Response) {
    ...
    const countNotifications = await this.service.getCountNotificationsByUserid(
      req.session.qexal.id,
    );
    const countNotif =
      countNotifications.result > 0
  ...
  async settingsPostForPassword(
    ...
    if (req.session.qexal) {
      if (
        demande.oldPass.length <= 3 ||
        demande.newpass.length <= 3 ||
        demande.confirm.length <= 3
      ) {
        return {
          etat: false,
  ...
  async removeDemande(@Body() demande: { ref: string }, @Request() req) {
    if (req.session.qexal && req.session.qexal.roleid === 3) {
      const update = await this.service.deleteDemande(
        parseInt(demande.ref, 10),
      );
  ...
  async repousserDemande(@Body() demande: { ref: string }, @Request() req) {
    if (req.session.qexal && req.session.qexal.roleid === 3) {
      const update = await this.service.repousserDemandeById(
        parseInt(demande.ref, 10),
      );
  ...
  async withdraw(@Request() req, @Res() res: Response) {
    ...
    let withdrawInAwait;
	  const allWithdraws = withdraws.map((withdraw) => {
    ...
    if(etatid === 1) {
		...
		return {id, ref, amount, userid, etatid, addressDestinate, etat, create_at: formatDateAndTime(create_at), updated_at: formatDateAndTime(updated_at)};
	  })
      let lastDemande = await this.service.getLastDemandeForSuivieByItem({
        userid: user.result.id,
        etatid: 2,
  ...
  async envoiereception(@Request() req, @Res() res: Response) {
    if (req.session.qexal && req.session.qexal.roleid === 3) {
      ...
      const inscription = await this.service.getUsersByItem({
        inscription: 2,
        isActive: true,
      });
      const intrusion = await this.service.getUsersByItem({
        inscription: 1,
        ref: '',
      });
      const inscriptionForLocked = await this.service.getUsersByItem({
        inscription: 2,
        isActive: false,
      });
      ...
      for (let i = 0; i < inscription.result.length; i++) {
        const forfait = await this.service.getForfaitById(
          inscription.result[i].forfaitid,
        );
        inscriptionFinal.push({
          ...inscription.result[i],
          montants:
            parseFloat(forfait.result.inscription) +
            parseFloat(forfait.result.nextGenForfaitMontant),
        });
      }
      for (let i = 0; i < inscriptionForLocked.result.length; i++) {
        const forfait = await this.service.getForfaitById(
          inscriptionForLocked.result[i].forfaitid,
        );
        inscriptionFinalForLocker.push({
          ...inscriptionForLocked.result[i],
          montants:
            parseFloat(forfait.result.inscription) +
            parseFloat(forfait.result.nextGenForfaitMontant),
        });
  ...
  async deleteUser(@Body() demande: { ref: string }, @Request() req) {
    if (req.session.qexal && req.session.qexal.roleid === 3) {
      ...
      const delAllNotification =
        await this.service.deleteAllNotificationsByUserid(id);
  ...
  async changeLink(@Request() req) {
    if (req.session.qexal) {
      ...
      const updateUser = await this.service.updateUser(req.session.qexal.id, {
        recovery,
      });
  ...
  async unlocked(@Body() demande: { ref: string }, @Request() req) {
    if (req.session.qexal && req.session.qexal.roleid === 3) {
      ...
      const unlockedDemande =
        await this.service.getLastDemandeForSuivieForUnlocked(id);
      if (validNumber.etat) {
        return {
          etat: true,
  ...
  @Post('doneFraude')
  async doneFraude(
    @Body() demande: { idFraude: string; idVictime: string; idSerial: string },
    @Request() req,
  ) {
    if (req.session.qexal && req.session.qexal.roleid === 3) {
      ...
      const clearUserFraude = await this.service.updateUser(idFraude, {
        isActive: true,
        motif: '',
        inscription: 1,
        ref: userInVictime.result.ref,
      });
      const incrimineVictime = await this.service.updateUser(idVictime, {
        isActive: false,
        inscription: 4,
        motif: 'Use txHash for other account',
      });
      const clearSerial = await this.service.updateSerialUser(idSerial, {
        isDone: true,
      });
  ...
  async updateForfaitInRetrograde(
    @Body() userinfo: { id: string },
    @Request() req,
  ) {
    if (req.session.qexal && req.session.qexal.roleid === 3) {
      const id = parseInt(userinfo.id, 10);
      const user = await this.service.getUserByItem({ id });
      const updateUser = await this.service.updateUser(id, {
        forfaitid: user.result.forfaitid - 1,
      });
      if (updateUser.etat) {
        return {
          etat: true,
  ...
  async renewLimit(
    @Body()
    demande: { id: string; limit: string; nextGenForfaitMontant: string },
    @Request() req,
  ) {
    if (req.session.qexal && req.session.qexal.roleid === 3) {
      ...
      const updateForfait = await this.service.updateForfait(id, {
        limit,
        nextGenForfaitMontant,
      });
  ...
  async delaccount(@Request() req, @Res() res: Response) {
    if (
      req.session.qexal &&
      req.session.qexal.roleid === 1 &&
      req.session.qexal.inscription === 2
    ) {
      const delAllNotification =
        await this.service.deleteAllNotificationsByUserid(req.session.qexal.id);
  ...
  async videNotif(@Request() req) {
    if (req.session.qexal) {
      const updateNotif = await this.service.updateNotificationsByUserid(
        req.session.qexal.id,
        {
          isOpen: true,
        },
  ...
  async faq(@Request() req, @Res() res: Response) {
    ...
    res
      ...
      .render('faq', {
        ...
        countNotif,
        notifications,
        title: `${req.session.qexal.name} - Faq`,
      });
  }
}
