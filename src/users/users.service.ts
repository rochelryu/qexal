...
import {
  formatedEndpointcrypto,
  generateRecoveryForHelp,
} from 'src/common/functions/helper';
import {
  MoreThanDate,
  MoreThanOrEqualDate,
  BetweenDate,
} from 'src/common/functions/date';
...
import {
  ADDRESS_BILLIONARY_INVESTMENT_FOR_SUBSRIPTION,
  ADDRESS_qexal_EXCHANGE,
  ADDRESS_qexal_INVESTMENT,
...
export class UsersService {
  ...
  constructor(
    ...
    private demandesRepository: Repository<DemandeEntity>,
    @InjectRepository(ForfaitEntity)
    private forfaitRepository: Repository<ForfaitEntity>,
    ...
    @InjectRepository(SchoolarshipEntity)
    private schoolarshipRepository: Repository<SchoolarshipEntity>,

    @InjectRepository(SerialUserEntity)
    private serialUserRepository: Repository<SerialUserEntity>,

    @InjectRepository(ControlCodeEntity)
    private controlCodeRepository: Repository<ControlCodeEntity>,
    @InjectRepository(WithdrawEntity)
    private withdrawRepository: Repository<WithdrawEntity>,
  ) {
    this.web3 = new Web3(
      'https://mainnet.infura.io/v3/bfa8430008fd4845b91e14eadf901c2b',
    );
    for (let i = 0; i < 3; i++) {
  ...
  async getUsersInWaitPayement(): Promise<ResponseProvider> {
    return new Promise(async (next) => {
      await this.usersRepository
        .find({ where: { retraitInWait: 1 }, order: { dateRetry: 'ASC' } })
        .then((result) => {
          next({ etat: true, result });
        })
        .catch((error) => {
  ...
  async getCountUserFollowInRange(
    dateMatrixCount: Date,
    parrainid: number,
  ): Promise<ResponseProvider> {
    return new Promise(async (next) => {
      const current = await this.usersRepository.query(
        'SELECT COUNT(id) as nombre FROM user_entity WHERE parrainid = ? AND validEntryInSysteme > ? AND accord = 1 AND filleulForMatrix = 1',
  ...
  async getAllAccountInWaitingRetraitForAnnulation(): Promise<ResponseProvider> {
    return new Promise(async (next) => {
      const current = await this.usersRepository.query(
        'select userreceptionid from traitement_entity where etatid = 1 or etatid = 2 group by userreceptionid',
      );
      next({ etat: true, result: current });
    });
  }

  async getUserFollowInRange(
    dateMatrixCount: Date,
    parrainid: number,
  ): Promise<ResponseProvider> {
    return new Promise(async (next) => {
      await this.usersRepository
        .query(
          'SELECT * FROM user_entity WHERE parrainid = ? AND validEntryInSysteme > ? AND accord = 1 AND filleulForMatrix = 1',
          [parrainid, dateMatrixCount],
        )
        .then((result) => {
          next({ etat: true, result });
  ...
  async getCountUsersConnectedToDay(
    toDay: Date = new Date(),
  ): Promise<ResponseProvider> {
    return new Promise(async (next) => {
  ...
  async getUserByQueryForRecupTrueRef(): Promise<ResponseProvider> {
    return new Promise(async (next) => {
      const info = await this.usersRepository.query(
        "select ref, id from user_entity where ref != '' and ref != 'old-'",
      );
  ...
  async regeneratePasswordUser(id: number): Promise<ResponseProvider> {
    return new Promise(async (next) => {
      await this.usersRepository
        .findOne({ where: { id } })
        .then(async (result) => {
  ...
  async updateUserForPayeSoldeGainIncrement(
    id: number,
    amount: number,
  ): Promise<ResponseProvider> {
    return new Promise(async (next) => {
  ...
  async demographieUser(): Promise<ResponseProvider> {
    return new Promise(async (next) => {
      await this.usersRepository
        .query(
          'SELECT COUNT(id) nombre, alpha2code FROM user_entity GROUP BY alpha2code',
        )
        .then((res) => {
          next({ etat: true, result: res });
        })
      ...
      await this.forfaitRepository
        ...
        .catch((error) =>
          next({
            etat: false,
            error: new Error(
              "l'enregistrement du forfait a échoué, veuillez ressayer plutard",
            ),
          }),
        );
    });
  ...
  async getCountDemandeByUserInRange(
    endContratTime: Date,
    userid: number,
  ): Promise<ResponseProvider> {
    return new Promise(async (next) => {
      const current = await this.demandesRepository.query(
        'SELECT COUNT(id) as nombre FROM demande_entity WHERE userid = ? AND create_at >= ?',
  ...
  async getCountDemandeByUserInRangeNoFilter(
    endContratTime: Date,
    userid: number,
  ): Promise<ResponseProvider> {
    return new Promise(async (next) => {
        ...
        .then((res) => {
          if (res.length > 1) {
            next({
              etat: false,
              error: new Error(
                "Vous avez déjà des demandes d'aide pas terminées",
              ),
            });
          } else {
            next({ etat: true, result: res });
  ...
  async getCountDemandeUserForPostNewDemande(
    userid: number,
  ): Promise<ResponseProvider> {
    return new Promise(async (next) => {
      await this.demandesRepository
        .count({ where: { userid } })
        .then(async (res) => {
          const valide = await this.demandesRepository.count({
            where: { userid },
          });
  ...
  async getDemandeWaitForAuto(
    userId: number,
    id: number,
  ): Promise<ResponseProvider> {
    return new Promise(async (next) => {
      await this.demandesRepository
        .query('SELECT * FROM demande_entity WHERE userid = ? AND id > ?', [
          userId,
          id,
        ])
        .then((res) => {
      ...
      await this.demandesRepository
        ...
        .then(async (result) => {
          if (result && result.accord === 2) {
            ...
            await result
              .save()
              .then((res) => next({ etat: true, result: res }))
              .catch((error) => next({ etat: false, error }));
          } else {
            next({
              etat: false,
  ...
  async getDemandeFinishedAndNoCheck(
    userid: number,
  ): Promise<ResponseProvider> {
    return new Promise(async (next) => {
      await this.demandesRepository
        // eslint-disable-next-line @typescript-eslint/camelcase
        .findOne({
          where: { etatid: 3, userid: Not(userid) },
          order: { updated_at: 'ASC' },
        })
        .then((res) => {
          if (res) {
            ...
            next({
              etat: false,
              error: new Error(
                "Aucune Demande est terminé et en Attente pour l'instant",
              ),
            });
          }
        })
  ...
  async getDemandeFinishedAndNoCheckFilterByForfait(
    userid: number,
    forfaitid: number,
  ): Promise<ResponseProvider> {
    return new Promise(async (next) => {
      await this.demandesRepository
        // eslint-disable-next-line @typescript-eslint/camelcase
        .findOne({
          where: { etatid: 3, forfaitid, userid: Not(userid) },
          order: { updated_at: 'ASC' },
        })
        .then((res) => {
          if (res) {
  ...
  async getAllDemandeFinishedAndNoCheck(
    forfaitid: number,
  ): Promise<ResponseProvider> {
    return new Promise(async (next) => {
      await this.demandesRepository
  ...
  async getLastDemandeForSuivieForUnlocked(
    userid: number,
  ): Promise<ResponseProvider> {
    return new Promise(async (next) => {
      await this.demandesRepository
  ...
  async setDemande(
    initDemande: FormatInitDemandeInterface,
  ): Promise<ResponseProvider> {
    return new Promise(async (next) => {
      await this.demandesRepository
        // eslint-disable-next-line @typescript-eslint/camelcase
        ...
        .catch((error) =>
          next({
            etat: false,
            error: new Error(
              "l'enregistrement de la demande a échoué, veuillez ressayer plutard",
            ),
          }),
        );
    });
  ...
  async getCountDemandeForStatToDay(
    forfaitid: number,
  ): Promise<ResponseProvider> {
    return new Promise(async (next) => {
      // eslint-disable-next-line @typescript-eslint/camelcase
      const current = await this.demandesRepository.count({
        where: {
          forfaitid,
          // eslint-disable-next-line @typescript-eslint/camelcase
      ...
      const month1 = await this.demandesRepository.count({
        where: {
          forfaitid,
          // eslint-disable-next-line @typescript-eslint/camelcase
      ...
      const month2 = await this.demandesRepository.count({
        where: {
          forfaitid,
          // eslint-disable-next-line @typescript-eslint/camelcase
  ...
  async getLatestNotifications(): Promise<ResponseProvider> {
    return new Promise(async (next) => {
      await this.notificationRepository
        .query(
          'SELECT notification_entity.content, notification_entity.type, user_entity.name, notification_entity.create_at FROM notification_entity LEFT JOIN user_entity ON notification_entity.userid= user_entity.id WHERE notification_entity.isOpen=1 ORDER BY notification_entity.id DESC LIMIT 15',
        )
        .then((res) => {
          next({ etat: true, result: res });
        })
  ...
  async deleteAllNotificationsByUserid(
    userid: number,
  ): Promise<ResponseProvider> {
    return new Promise(async (next) => {
      await this.notificationRepository
  ...
  async getCountNotificationsByUserid(
    userid: number,
  ): Promise<ResponseProvider> {
    return new Promise(async (next) => {
      await this.notificationRepository
  ...
  async verifyTxHash(
    ...
    ref: string,
    value: number,
    destinationAddress: string = ADDRESS_qexal_INVESTMENT,
  ): Promise<ResponseProvider> {
          ...
          async (response) => {
            ...
            if (
              ...
              destination.trim().toLocaleLowerCase() ===
                destinationAddress.toLocaleLowerCase().trim()
            ) {
              ...
              if (value - montantUsd <= 1) {
                next({
                  etat: true,
                  result: { ref, montant: 0, montant_net_send: montantUsd },
                });
              } else {
                next({
                  etat: false,
                  result: {
                    ref,
                    montant: value - montantUsd,
                    montant_net_send: montantUsd,
                  },
                  error: new Error('incomplete transaction'),
                });
              }
            }
            ...
            // }
              ...
              next({
                etat: false,
                error: new Error(
                  'this transaction has failed, please make a new transaction to sign your contract',
                ),
              });
            }
          },
  ...
  async getValueInCurrencie(
    fromCurrencie: string,
    toCurrencie: string,
  ): Promise<ResponseProvider> {
        ...
        .then(
          ...
          async (response) => {
            ...
            next({
              etat: true,
              result: {
                fromCurrencie: fromCurrencie.toLocaleUpperCase(),
                toCurrencie: toCurrencie.toLocaleUpperCase(),
    ...
    try {
      ...
      if (block.transactions.length > 0) {
        ...
        const transaction = await this.web3.eth.getTransaction(
          latestTransactionHash.toString(),
        );
        ...
        return {
          etat: false,
          error: new Error('No transactions found in the latest block.'),
        };
      }
    } catch (error) {
      ...
      AxiosInstance.get(url)
        .then(
          ...
          async (response) => {
            ...
            $('.table.table-hover.table-align-middle.mb-0 tr').each(function (
              index: number,
              element: any,
            ) {
              const txHash = $(element)
                .find('a.myFnExpandBox_searchVal')
                .text();
              const typeTransaction = $(element)
  ...
  async verifyTxHashAllTypeCrypto(
    ref: string,
    value: number,
    destinationAddress: string = ADDRESS_BILLIONARY_INVESTMENT_FOR_SUBSRIPTION,
    endpointcrypto: string = 'bitcoin',
  ): Promise<ResponseProvider> {
    if (endpointcrypto.toLocaleLowerCase().indexOf('binance coin') !== -1) {
      return await this.verifyTxHashForExchangeBinanceCoin(
        ref,
        value,
        destinationAddress,
      );
      ...
      endpointcrypto.toLocaleLowerCase().indexOf('usdt - trc20') !== -1
    ) {
      return await this.verifyTxHashForTronx(
        ref,
        value,
        destinationAddress,
        true,
      ...
      endpointcrypto.toLocaleLowerCase().indexOf('shiba inu - erc20') !== -1
    ) {
      return await this.verifyTxHashForExchangeShibaInuErc20(
        ref,
        value,
        destinationAddress,
      );
    } else if (
      endpointcrypto.toLocaleLowerCase().indexOf('shibnobi shinja - erc20') !==
      -1
    ) {
      return await this.verifyTxHashForExchangeShibnobiShinjaErc20(
        ref,
        value,
        destinationAddress,
      );
    } else if (
      endpointcrypto.toLocaleLowerCase().indexOf('radio caca v2 - erc20') !== -1
    ) {
      return await this.verifyTxHashForExchangeUniversalClassicErc20(
        ref,
        value,
        destinationAddress,
        'Radio Caca V... (RACA)',
      ...
      endpointcrypto.toLocaleLowerCase().indexOf('shiba inu - bep20') !== -1
    ) {
      return await this.verifyTxHashForExchangeShibaInuBep20(
        ref,
        value,
        destinationAddress,
      );
    } else if (endpointcrypto.toLocaleLowerCase().indexOf('pappay') !== -1) {
      return await this.verifyTxHashForExchangePappayBep20(
        ref,
        value,
        destinationAddress,
      );
    } else if (endpointcrypto.toLocaleLowerCase().indexOf('metacity') !== -1) {
      return await this.verifyTxHashForExchangeMetaCityBep20(
        ref,
        value,
        destinationAddress,
      );
    } else if (
      endpointcrypto.toLocaleLowerCase().indexOf('silva token') !== -1
    ) {
      return await this.verifyTxHashForExchangeUniversalClassicBep20(
        ref,
        value,
        destinationAddress,
        'Silva Token (SILVA)',
      ...
      endpointcrypto.toLocaleLowerCase().indexOf('baby musk coin') !== -1
    ) {
      return await this.verifyTxHashForExchangeUniversalClassicBep20(
        ref,
        value,
      ...
      endpointcrypto.toLocaleLowerCase().indexOf('little rabbit') !== -1
    ) {
      return await this.verifyTxHashForExchangeUniversalClassicBep20(
        ref,
        value,
      ...
      endpointcrypto.toLocaleLowerCase().indexOf('radio caca v2 - bep20') !== -1
    ) {
      return await this.verifyTxHashForExchangeUniversalClassicBep20(
        ref,
        value,
      ...
      return await this.verifyTxHashForExchangeSaitamaErc20(
        ref,
        value,
        destinationAddress,
      );
      ...
      return await this.verifyTxHashForExchangeLittleRabbitBep20(
        ref,
        value,
        destinationAddress,
      );
    } else if (
      endpointcrypto.toLocaleLowerCase().indexOf('ariva - bep20') !== -1
    ) {
      return await this.verifyTxHashForExchangeArivaBep20(
        ref,
        value,
        destinationAddress,
      );
    } else if (endpointcrypto.toLocaleLowerCase().indexOf('pitbull') !== -1) {
      return await this.verifyTxHashForExchangePitBull(
        ref,
        value,
        destinationAddress,
      );
    } else if (endpointcrypto.toLocaleLowerCase().indexOf('callisto') !== -1) {
      return await this.verifyTxHashForExchangeCallisto(
        ref,
        value,
        destinationAddress,
      );
    } else if (endpointcrypto.toLocaleLowerCase().indexOf('luna') !== -1) {
      return await this.verifyTxHashForExchangeLuna(
        ref,
        value,
        destinationAddress,
      );
    } else if (endpointcrypto.toLocaleLowerCase().indexOf('baby doge') !== -1) {
      return await this.verifyTxHashForExchangeBabyDogeBep20(
        ref,
        value,
        destinationAddress,
      );
    } else if (
      endpointcrypto.toLocaleLowerCase().indexOf('binance smart chain') !== -1
    ) {
      return await this.verifyTxHashForExchangeSmartChain(
        ref,
        value,
        destinationAddress,
      );
    } else if (endpointcrypto.toLocaleLowerCase().indexOf('polkadot') !== -1) {
      return await this.verifyTxHashForExchangePolkadot(
        ref,
        value,
        destinationAddress,
      );
    } else if (endpointcrypto.toLocaleLowerCase().indexOf('cardano') !== -1) {
      return await this.verifyTxHashForExchangeCardano(
        ref,
        value,
        destinationAddress,
      );
    } else if (endpointcrypto.toLocaleLowerCase().indexOf('ripple') !== -1) {
      return await this.verifyTxHashForExchangeRipple(
        ref,
        value,
        destinationAddress,
      );
    } else if (endpointcrypto.toLocaleLowerCase().indexOf('tezos') !== -1) {
      return await this.verifyTxHashForExchangeTezos(
        ref,
        value,
        destinationAddress,
      );
    } else {
      return await this.verifyTxHashForOtherCrypto(
        ref,
        value,
        destinationAddress,
        endpointcrypto,
  ...
  async verifyAddressForOtherCrypto(
    address: string,
    endpointcrypto: string,
  ): Promise<ResponseProvider> {
  ...
  async verifyTxHashForOtherCrypto(
    ref: string,
    value: number,
    destinationAddress: string = ADDRESS_BILLIONARY_INVESTMENT_FOR_SUBSRIPTION,
    endpointcrypto: string = 'bitcoin',
      ...
      AxiosInstance.get(url)
        .then(
          ...
          (response) => {
            ...
            const montant = isExchange
              ? json.data[ref].transaction.output_total / 100000000
              : json.data[ref].transaction.output_total_usd;
            ...
            if (state) {
              if (destination.trim() === destinationAddress) {
                if (value <= montant) {
                  next({
                    etat: true,
                    result: { ref, montant: 0, montant_net_send: montant },
                  });
                } else {
                  next({
                    etat: false,
                ...
                next({
                  etat: false,
                  error: new Error(
                    'Incorrect destination address. This transaction has failed, please make a new transaction to sign your contract',
                  ),
                });
              }
            } else {
              next({
                etat: false,
                error: new Error(
                  'this transaction is still in progress so please reach for it to be completed before entering the TxHash code',
                ),
              });
            }
          },
  ...
  async getLastTxHashForCryptoInBlockchair(
    address: string,
    endpointcrypto: string = 'bitcoin',
  ): Promise<ResponseProvider> {
    const endPoint = formatedEndpointcrypto(endpointcrypto);
        ...
        .then(
          ...
          (response) => {
            ...
            if (state) {
              next({
                etat: true,
                result: { ref: json.data[address].transactions[0].hash },
              });
            } else {
              next({
                etat: false,
                error: new Error(
                  'Not transaction at this address for START UP',
                ),
              });
            }
          },
  ...
  async verifyTxHashForOtherCryptoWithoutValue(
    ref: string,
    ...
    return new Promise(async (next) => {
      AxiosInstance.get(url)
        .then((response) => {
          ...
          let destination = '';
          if (endPoint === 'bitcoin') {
            const recipient = json.data[ref].outputs.filter(
              (value) => value.recipient === destinationAddress,
            );
          ...
          if (state) {
            if (destination.trim() === destinationAddress) {
              ...
              next({
                etat: false,
                error: new Error(
                  "L'adresse de destination est incorrecte. Cette transaction a échoué",
                ),
              });
            }
          } else {
            next({
              etat: false,
              error: new Error(
                "Cette transaction est toujours en cours, veuillez attendre qu'elle soit terminée avant d'entrer le code TxHash.",
              ),
            });
          }
        })
  ...
  async verifyTxHashForExchangeCardano(
    ref: string,
    value: number,
    destinationAddress: string,
  ): Promise<ResponseProvider> {
      ...
      AxiosInstance.get(url)
        .then(
          ...
          (response) => {
            ...
            const montant =
              parseFloat(json.data[ref].transaction.ctsTotalOutput.getCoin) /
              1000000;
            const destination =
              json.data[ref].transaction.ctsOutputs[0].ctaAddress;
  ...
  async verifyTxHashForExchangeRipple(
    ref: string,
    value: number,
    destinationAddress: string,
  ): Promise<ResponseProvider> {
      ...
      AxiosInstance.get(url)
        .then(
          ...
          (response) => {
            ...
            const montantTronx = !usdt
              ? json.contractData.amount / 1000000
              : parseInt(json.trc20TransferInfo.amount_str, 10) / 1000000;
            const destination = !usdt
              ? json.toAddress
              : json.trc20TransferInfo.to_address;

            if (state && destination.trim() === destinationAddress) {
              if (value <= montantTronx) {
                next({
                  etat: true,
                  result: { ref, montant: 0, montant_net_send: montantTronx },
                });
              } else {
                next({
                  etat: false,
                  result: {
                    ref,
                    montant: value - montantTronx,
                    montant_net_send: montantTronx,
                  },
                  error: new Error('incomplete transaction'),
                });
              }
  ...
  async verifyTxHashForExchangeTezos(
    ref: string,
    value: number,
    destinationAddress: string,
  ): Promise<ResponseProvider> {
      ...
      AxiosInstance.get(url)
        .then(
          ...
          async (response) => {
            ...
            if (
              state.text().trim() === 'Success' &&
              destination.text().trim() === ADDRESS_qexal_EXCHANGE
            ) {
  ...
  async verifyTxHashForExchangeShibaInuErc20(
    ref: string,
    value_shiba: number,
    address_shiba: string,
  ): Promise<ResponseProvider> {
    const url = `https://etherscan.io/tx/${ref}`; // URL we're scraping
    const AxiosInstance = axios.create();
    return new Promise(async (next) => {
      AxiosInstance.get(url)
        .then(
          ...
          async (response) => {
            ...
            const state = $('span.u-label.u-label--sm');
            const destination = $(
              '#wrapperContent .media-body span.hash-tag.text-truncate.hash-tag-custom-to.tooltip-address',
            );
            const montant = $('#wrapperContent .media-body span:nth-child(6)');
            const shibaTokenVerify = $(
              '#wrapperContent .media-body a:last-child',
            );
            const valueShiba = parseFloat(montant.text().trim());

            if (
              state.text().trim() === 'Success' &&
              destination.text().trim() === address_shiba &&
              shibaTokenVerify.text().trim().indexOf('SHIBA INU (SHIB)') !== -1
            ) {
  ...
  async verifyTxHashForExchangeShibnobiShinjaErc20(
    ref: string,
    value_shinja: number,
    address_shinja: string,
  ): Promise<ResponseProvider> {
    const url = `https://etherscan.io/tx/${ref}`; // URL we're scraping
    const AxiosInstance = axios.create();
    return new Promise(async (next) => {
      AxiosInstance.get(url)
        .then(
          ...
          async (response) => {
            ...
            const montant = $('#wrapperContent .media-body span:nth-child(6)');
            const shinjaTokenVerify = $(
              '#wrapperContent .media-body a:last-child',
            );
            const valueShinja = parseFloat(
              montant.text().trim().replaceAll(',', ''),
            );

            if (
              state.text().trim() === 'Success' &&
              destination.text().trim() === address_shinja &&
              shinjaTokenVerify.text().trim().indexOf('Shibnobi (SHINJA)') !==
                -1
            ) {
  ...
  async verifyTxHashForExchangeSaitamaErc20(
    ref: string,
    value_saitama: number,
    address_saitama: string,
  ): Promise<ResponseProvider> {
    const url = `https://etherscan.io/tx/${ref}`; // URL we're scraping
    const AxiosInstance = axios.create();
    return new Promise(async (next) => {
      AxiosInstance.get(url)
        .then(
          ...
          async (response) => {
            ...
            if (
              state.text().trim() === 'Success' &&
              destination.text().trim() === address_saitama &&
              tokenVerify.text().trim().indexOf('Saitama Inu (') !== -1
            ) {
  ...
  async verifyTxHashForExchangeCallisto(
    ref: string,
    value_callisto: number,
    address_callisto: string,
  ): Promise<ResponseProvider> {
      ...
      AxiosInstance.get(url)
        .then(
          ...
          async (response) => {
            ...
            const $ = cheerio.load(html);
            const state = $(
              'div.d-flex.flex-row.justify-content-start.text-muted span:nth-child(2)',
            );
            const destination = $(
              'span.d-block.mb-2.text-muted a:last-child span.d-none.d-md-none.d-xl-inline',
            );
            ...
            if (
              state.text().trim() === 'Success' &&
              destination.text().trim() === address_callisto
            ) {
  ...
  async verifyTxHashForExchangeLuna(
    ref: string,
    value_luna: number,
    address_luna: string,
  ): Promise<ResponseProvider> {
  ...
  async verifyTxHashForExchangeShibaInuBep20(
    ref: string,
    value_shiba: number,
    address_shiba: string,
  ): Promise<ResponseProvider> {
  ...
  async verifyTxHashForExchangeArivaBep20(
    ref: string,
    value_ariva: number,
    address_ariva: string,
  ): Promise<ResponseProvider> {
    const url = `https://bscscan.com/tx/${ref}`; // URL we're scraping
    const AxiosInstance = axios.create();
    return new Promise(async (next) => {
      AxiosInstance.get(url)
        .then(
          ...
          async (response) => {
            ...
            const montant = $('#wrapperContent .media-body span:nth-child(6)');
            const arivaTokenVerify = $(
              '#wrapperContent .media-body a:last-child',
            );
            const valueAriva = parseFloat(
              montant.text().trim().replaceAll(',', ''),
            );

            if (
              state.text().trim() === 'Success' &&
              destination.text().trim() === address_ariva &&
              arivaTokenVerify.text().trim().indexOf('ARIVA (ARV)') !== -1
            ) {
  ...
  async verifyTxHashForExchangeMetaCityBep20(
    ref: string,
    value_metaCity: number,
    address_metaCity: string,
  ): Promise<ResponseProvider> {
    const url = `https://bscscan.com/tx/${ref}`; // URL we're scraping
    const AxiosInstance = axios.create();
    return new Promise(async (next) => {
      AxiosInstance.get(url)
        .then(
          ...
          async (response) => {
            ...
            const montant = $('#wrapperContent .media-body span:nth-child(6)');
            const metaCityTokenVerify = $(
              '#wrapperContent .media-body a:last-child',
            );
            const valueMetaCity = parseFloat(
              montant.text().trim().replaceAll(',', ''),
            );
            // this.logger.log({
            // 	state: state.text().trim(),
            ...
            if (
              state.text().trim() === 'Success' &&
              destination.text().trim() === address_metaCity &&
              metaCityTokenVerify
                .text()
                .trim()
                .indexOf('Meta City (METACI...)') !== -1
            ) {
  ...
  async verifyTxHashForExchangeUniversalClassicBep20(
    ref: string,
    value_crypto: number,
    address_crypto: string,
    nameCryptoForVerify: string,
  ): Promise<ResponseProvider> {
    const url = `https://bscscan.com/tx/${ref}`; // URL we're scraping
    const AxiosInstance = axios.create();
    return new Promise(async (next) => {
      AxiosInstance.get(url)
        .then(
          ...
          async (response) => {
            ...
            const montant = $('#wrapperContent .media-body span:nth-child(6)');
            const nameTokenVerify = $(
              '#wrapperContent .media-body a:last-child',
            );
            const valueCrypto = parseFloat(
              montant.text().trim().replaceAll(',', ''),
            );
            /*this.logger.log({
					state: state.text().trim(),
            ...
            if (
              state.text().trim() === 'Success' &&
              destination.text().trim() === address_crypto &&
              nameTokenVerify
                .text()
                .trim()
                .indexOf(nameCryptoForVerify.trim()) !== -1
            ) {
  ...
  async verifyTxHashForExchangeUniversalClassicErc20(
    ref: string,
    value_crypto: number,
    address_crypto: string,
    nameCryptoForVerify: string,
  ...
  async verifyTxHashForExchangePappayBep20(
    ref: string,
    value_pappay: number,
    address_pappay: string,
  ): Promise<ResponseProvider> {
    const url = `https://bscscan.com/tx/${ref}`; // URL we're scraping
    const AxiosInstance = axios.create();
    return new Promise(async (next) => {
      AxiosInstance.get(url)
        .then(
          ...
          async (response) => {
            ...
            const state = $('span.u-label.u-label--sm');
            const destination = $(
              '#wrapperContent li:last-child .media-body span.hash-tag.text-truncate.hash-tag-custom-to.tooltip-address',
            );
            const montant = $(
              '#wrapperContent li:last-child .media-body span:nth-child(6)',
            );
            const pappayTokenVerify = $(
              '#wrapperContent li:last-child .media-body a:last-child',
            );
            const valuePappay = parseFloat(
              montant.text().trim().replaceAll(',', ''),
            );

            if (
              state.text().trim() === 'Success' &&
              destination.text().trim() === address_pappay &&
              pappayTokenVerify.text().trim().indexOf('PAPPAY (PAPPAY)') !== -1
            ) {
  ...
  async verifyTxHashForExchangePitBull(
    ref: string,
    value_pitbull: number,
    address_pitbull: string,
  ): Promise<ResponseProvider> {
    const url = `https://bscscan.com/tx/${ref}`; // URL we're scraping
    const AxiosInstance = axios.create();
    return new Promise(async (next) => {
      AxiosInstance.get(url)
        .then(
          ...
          async (response) => {
            ...
            const montant = $('#wrapperContent .media-body span:nth-child(6)');
            const pitbullTokenVerify = $(
              '#wrapperContent .media-body a:last-child',
            );
            const valuePitbull = parseFloat(
              montant.text().trim().replaceAll(',', ''),
            );

            if (
              state.text().trim() === 'Success' &&
              destination.text().trim() === address_pitbull &&
              pitbullTokenVerify.text().trim().indexOf('Pitbull (PIT)') !== -1
            ) {
  ...
  async verifyTxHashForExchangeBabyDogeBep20(
    ref: string,
    value_babydoge: number,
    address_babydoge: string,
  ): Promise<ResponseProvider> {
    const url = `https://bscscan.com/tx/${ref}`; // URL we're scraping
    const AxiosInstance = axios.create();
    return new Promise(async (next) => {
          ...
          async (response) => {
            ...
            next({
              ...
              result: {
                ...
                valueBabyDoge,
                montant,
                tokenVerify: tokenVerify.text().trim(),
                value_babydoge,
                address_babydoge,
              },
            });
            ...
            if (
              state.text().trim() === 'Success' &&
              destination.text().trim() === address_babydoge &&
              tokenVerify.text().trim().indexOf('Baby Doge') !== -1
            ) {
  ...
  async verifyTxHashForExchangeLittleRabbitBep20(
    ref: string,
    value_little: number,
    address_little: string,
  ): Promise<ResponseProvider> {
    const url = `https://bscscan.com/tx/${ref}`; // URL we're scraping
    const AxiosInstance = axios.create();
    return new Promise(async (next) => {
      AxiosInstance.get(url)
        .then(
          ...
          async (response) => {
            ...
            if (
              state.text().trim() === 'Success' &&
              destination.text().trim() === address_little &&
              tokenVerify.text().trim().indexOf('(LTRBT)') !== -1
            ) {
  ...
  async verifyTxHashForExchangePolkadot(
    ref: string,
    value_polkadot: number,
    address_polkadot: string,
  ): Promise<ResponseProvider> {
    const url = `https://bscscan.com/tx/${ref}`; // URL we're scraping
    const AxiosInstance = axios.create();
    return new Promise(async (next) => {
      AxiosInstance.get(url)
        .then(
          ...
          async (response) => {
            ...
            if (
              state.text().trim() === 'Success' &&
              destination.text().trim() === address_polkadot &&
              polkadotTokenVerify
                .text()
                .trim()
                .toLowerCase()
  ...
  async verifyTxHashForExchangeSmartChain(
    ref: string,
    value_bnb: number,
    address_bnb: string,
  ): Promise<ResponseProvider> {
      ...
      AxiosInstance.get(url)
        .then(
          ...
          async (response) => {
            ...
            const valueDollars = $('#ContentPlaceHolder1_spanValue > button')
              .text()
              .trim()
              .split('($')[1];
            ...
            if (
              state.text().trim() === 'Success' &&
              destination.text().trim() === address_bnb
            ) {
  ...
  async verifyTxHashForExchangeBinanceCoin(
    ref: string,
    value_binance: number = 0,
    addresCrypto: string,
  ): Promise<ResponseProvider> {
      ...
      AxiosInstance.get(url)
        .then(
          ...
          async (response) => {
            ...
            const $ = cheerio.load(html);
            const state = $(
              'div.DetailCard__Row-sc-1gna34z-3.primary-color.fxoDIt.FlexBox__StyledFlexBox-ixcd3u-0.cxfWkp:nth-child(2) div.text-wrap > span',
            );
            const montant = $(
              'div.DetailCard__Row-sc-1gna34z-3.primary-color.fxoDIt.FlexBox__StyledFlexBox-ixcd3u-0.cxfWkp:last-child div',
            );
            const destination = $(
              'div.DetailCard__Row-sc-1gna34z-3.primary-color.fxoDIt.FlexBox__StyledFlexBox-ixcd3u-0.cxfWkp:nth-child(10) a',
            );
            const valueBinanceCoin = parseFloat(montant.text().trim());

            if (
              state.text().trim() === 'Success' &&
              destination.text().trim().toLocaleLowerCase() ===
                addresCrypto.toLocaleLowerCase()
            ) {
      ...
      await this.serialUserRepository
        ...
        .then(async (res) => {
          if (res) {
            next({
              etat: false,
              error: new Error('This serial already exist'),
            });
          } else {
            await this.serialUserRepository
      ...
      await this.serialUserRepository
        ...
        .catch((error) =>
          next({
            etat: false,
            error: new Error(
              "l'enregistrement du serial a échoué, veuillez ressayer plutard",
            ),
          }),
        );
    });
      ...
      await this.controlCodeRepository
        ...
        .catch((error) =>
          next({
            etat: false,
            error: new Error(
              "l'enregistrement du controlCode a échoué, veuillez ressayer plutard",
            ),
          }),
        );
    });
  ...
  // For Withdraws
  async getAllWithdrawsByItem(item): Promise<ResponseProvider> {
    return new Promise(async (next) => {
      await this.withdrawRepository
        .find({ where: item, relations: ['etat'], order: {create_at: 'DESC'} })
        .then((result) => {
          if (result) {
            next({ etat: true, result });
          } else {
  ...
  async getWithdrawByItem(item): Promise<ResponseProvider> {
    return new Promise(async (next) => {
      await this.withdrawRepository
        .findOne({ where: item, relations: ['etat', 'user'] })
        .then((result) => {
          if (result) {
            next({ etat: true, result });
          } else {
  ...
  async verifyIfUserAlreadySeeMovie(
    movieid: number,
    userid: number,
  ): Promise<ResponseProvider> {
    return new Promise(async (next) => {
